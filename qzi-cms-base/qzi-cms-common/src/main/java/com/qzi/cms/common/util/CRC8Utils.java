...
public class CRC8Utils {
    ...
    public static byte crc8(byte[] msg_ptr, int len) {
        byte crc = 0;
        for (int j = 0; j < msg_ptr.length; j++) {
            int value = (crc^msg_ptr[j]) & 0x00ff;
            crc = CRC8_TAB[value];
        }
    ...
    public static byte[] encode(byte[] msg_ptr, byte key) {
        byte[] data = new byte[msg_ptr.length+1];

        data[0] = key;
        for (int j = 0; j < msg_ptr.length; j++) {
            data[j+1] = (byte) (msg_ptr[j] ^ key);
        }

        return data;
    }

    public static byte crc8_Reverse(byte[] msg_ptr, int len) {
        byte crc = 0;
        for (int j = msg_ptr.length-1; j >=0; j--) {
            int value = (crc^msg_ptr[j]) & 0x00ff;
            crc = CRC8_TAB[value];
        }
    ...
    public static int CRC16_Check(byte Pushdata[])  {
        int Reg_CRC=0xffff;
        int temp;
        int i,j;
